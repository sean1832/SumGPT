...
with file_handler:
    ...
    upload_file = upload_file_emtpy.file_uploader(
        "📁 Upload your file", type=["txt", "pdf", "docx", "md"]
    )
    if youtube_link:
    ...
    if file_content:
        ...
        if st.button("🚀 Run", disabled=exceeded):
            ...
            if not API_KEY and not GPT.misc.validate_api_key(API_KEY):
                st.error(
                    "❌ Please enter a valid [OpenAI API key](https://beta.openai.com/account/api-keys)."
                )
            else:
        ...
        if rec_responses is not None:
            ...
            if finish_reason_rec == "length":
                st.warning(
                    "⚠️Result cut off due to length. Consider increasing the [Max Tokens Chunks] parameter."
                )
